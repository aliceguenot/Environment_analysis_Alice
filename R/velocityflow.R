...
flow <- function(arms_id, meta_and_data, data_flow) {
  ...
  nt <- dim(time)
  lswt_array <- ncvar_get(dat.ncfd, "uo") #East
  dim(lswt_array)
  ...
  cowplot::plot_grid(p1,
                     ...
                     ncol = 2, 
                     nrow = 3)
  path_to_flowcurv <- paste0("outputs/flowcurvE_", arms_id,".pdf")
  ...
  path_to_F <- paste0("outputs/flow_", arms_id, ".csv")
  ...
  rast_flow_name <- paste0("rast_flow_", arms_id, ".pdf")
  rast_flow_path <- here::here("outputs", rast_flow_name)
  pdf(file =  rast_flow_path, width = 10, height = 10)
  
  raster::plot(rast.m, main = "Raster de la moyenne courants sur un an avec les sites RUNA (légende en m)")
  ...
  flow_all_sites_E <- flow_all_sites
  
  path_to_FE_full <- paste0("outputs/flowE_full_", arms_id, ".csv")
  
  write.table(flow_all_sites_E, 
              file = path_to_FE_full, 
              dec = ",", 
              sep = ";",
              row.names = FALSE)
  
  
  return(c(path_to_F,path_to_FE_full))
} 
